    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.levels.update", [$level->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $level->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="number" name="lat" id="lat" value="{{ old('lat', $level->lat) }}" step="0.00000001" required>
                @if($errors->has('lat'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="number" name="lng" id="lng" value="{{ old('lng', $level->lng) }}" step="0.00000001" required>
                @if($errors->has('lng'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ old('key', $level->key) }}" required>
                @if($errors->has('key'))
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.levels.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $level->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
...
<script>
    ...
    url: '{{ route('admin.levels.storeMedia') }}',
    maxFilesize: 2, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
