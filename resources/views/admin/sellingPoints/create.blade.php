...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.sellingPoint.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.selling-points.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="icon_code">{{ trans('cruds.sellingPoint.fields.icon_code') }}</label>
                <input class="form-control {{ $errors->has('icon_code') ? 'is-invalid' : '' }}" type="text" name="icon_code" id="icon_code" value="{{ old('icon_code', '') }}">
