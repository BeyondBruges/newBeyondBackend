...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-side-quests.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('side-quest') ? 'is-invalid' : '' }}" name="side_quest_id" id="side-quest_id" required>
                    @foreach($sidequests as $id => $entry)
                        <option value="{{ $entry->id }}" {{ old('side-quest_id') == $id ? 'selected' : '' }}>{{ $entry->name }}</option>
                    @endforeach
                </select>
