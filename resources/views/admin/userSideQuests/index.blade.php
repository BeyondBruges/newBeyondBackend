...
@can('level_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.user-side-quests.create') }}">
                {{ trans('global.add') }} User Side Quest
            </a>
        </div>
    </div>
@endcan
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Level">
                ...
                <tbody>
                    @foreach($usersidequests as $key => $level)
                        <tr data-entry-id="{{ $level->id }}">
                            ...
                            <td>
                                ...
                                @can('level_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.user-side-quests.edit', $level->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
