...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.b-land-marks.update", [$bLandMark->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.bLandMark.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $bLandMark->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address" id="address" required>{{ old('address', $bLandMark->address) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="number" name="lat" id="lat" value="{{ old('lat', $bLandMark->lat) }}" step="0.00000001" required>
                @if($errors->has('lat'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="number" name="lng" id="lng" value="{{ old('lng', $bLandMark->lng) }}" step="0.00000001" required>
                @if($errors->has('lng'))
