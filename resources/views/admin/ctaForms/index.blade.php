...
@can('cta_form_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.cta-forms.create') }}">
                ...
                <tbody>
                    @foreach($ctaForms as $key => $ctaForm)
                        <tr data-entry-id="{{ $ctaForm->id }}">
                            ...
                            <td>
                                @can('cta_form_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.cta-forms.show', $ctaForm->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('cta_form_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.cta-forms.edit', $ctaForm->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('cta_form_delete')
                                    <form action="{{ route('admin.cta-forms.destroy', $ctaForm->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cta_form_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.cta-forms.massDestroy') }}",
