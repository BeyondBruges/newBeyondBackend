...
@can('contact_text_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.contact-texts.create') }}">
                        ...
                        <tr data-entry-id="{{ $contactText->id }}">
                            ...
                            <td>
                                @can('contact_text_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.contact-texts.show', $contactText->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('contact_text_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.contact-texts.edit', $contactText->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('contact_text_delete')
                                    <form action="{{ route('admin.contact-texts.destroy', $contactText->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('contact_text_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.contact-texts.massDestroy') }}",
