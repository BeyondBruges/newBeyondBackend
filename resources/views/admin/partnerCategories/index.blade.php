...
@can('dialogue_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.partnerCategories.create') }}">
                    ...
                    @foreach($categories as $key => $category)
                        <tr data-entry-id="{{ $category->id }}">
                            ...
                            <td>
                                ...
                                @can('dialogue_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.partnerCategories.edit', $category->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('dialogue_delete')
                                    <form action="{{ route('admin.partnerCategories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
