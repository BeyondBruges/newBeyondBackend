...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.side-quests.update",$sidequest)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.level.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ $sidequest->name }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('key') ? 'is-invalid' : '' }}" type="text" name="key" id="key" value="{{ $sidequest->key }}" required>
                @if($errors->has('key'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('level') ? 'is-invalid' : '' }}" name="level_id" id="level_id" required>
                    @foreach($levels as $id => $entry)
                        ...
                        <option value="{{ $entry->id }}" {{ $sidequest->id == $id ? 'selected' : '' }}>{{ $entry->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('level'))
