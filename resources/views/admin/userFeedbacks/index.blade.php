...
@can('user_feedback_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.user-feedbacks.create') }}">
                        ...
                        <tr data-entry-id="{{ $userFeedback->id }}">
                            ...
                            <td>
                                @can('user_feedback_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.user-feedbacks.show', $userFeedback->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('user_feedback_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.user-feedbacks.edit', $userFeedback->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('user_feedback_delete')
                                    <form action="{{ route('admin.user-feedbacks.destroy', $userFeedback->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_feedback_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.user-feedbacks.massDestroy') }}",
