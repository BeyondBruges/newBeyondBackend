        ...
        <div class="col-md-12">
            @can('app_menu_button_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.app-menu-buttons.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $appMenuButton->id }}">
                                        ...
                                        <td>
                                            @can('app_menu_button_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.app-menu-buttons.show', $appMenuButton->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('app_menu_button_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.app-menu-buttons.edit', $appMenuButton->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('app_menu_button_delete')
                                                <form action="{{ route('frontend.app-menu-buttons.destroy', $appMenuButton->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('app_menu_button_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.app-menu-buttons.massDestroy') }}",
