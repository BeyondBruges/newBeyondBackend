    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.analytics.update", [$analytic->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.analytic.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $analytic->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="value" id="value" value="{{ old('value', $analytic->value) }}">
                            @if($errors->has('value'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('value') }}
                                </div>
