        ...
        <div class="col-md-12">
            @can('analytic_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.analytics.create') }}">
                            ...
                            <tbody>
                                @foreach($analytics as $key => $analytic)
                                    <tr data-entry-id="{{ $analytic->id }}">
                                        ...
                                        <td>
                                            @can('analytic_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.analytics.show', $analytic->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('analytic_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.analytics.edit', $analytic->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('analytic_delete')
                                                <form action="{{ route('frontend.analytics.destroy', $analytic->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('analytic_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.analytics.massDestroy') }}",
