        ...
        <div class="col-md-12">
            @can('dynamic_coupon_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.dynamic-coupons.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $dynamicCoupon->id }}">
                                        ...
                                        <td>
                                            @can('dynamic_coupon_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.dynamic-coupons.show', $dynamicCoupon->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('dynamic_coupon_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.dynamic-coupons.edit', $dynamicCoupon->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('dynamic_coupon_delete')
                                                <form action="{{ route('frontend.dynamic-coupons.destroy', $dynamicCoupon->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('dynamic_coupon_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.dynamic-coupons.massDestroy') }}",
