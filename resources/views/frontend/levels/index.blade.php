                            ...
                            <tbody>
                                @foreach($levels as $key => $level)
                                    <tr data-entry-id="{{ $level->id }}">
                                        ...
                                        <td>
                                            @can('level_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.levels.show', $level->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('level_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.levels.edit', $level->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('level_delete')
                                                <form action="{{ route('frontend.levels.destroy', $level->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('level_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.levels.massDestroy') }}",
