        ...
        <div class="col-md-12">
            @can('hotspot_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.hotspots.create') }}">
                            ...
                            <tbody>
                                @foreach($hotspots as $key => $hotspot)
                                    <tr data-entry-id="{{ $hotspot->id }}">
                                        ...
                                        <td>
                                            @can('hotspot_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.hotspots.show', $hotspot->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('hotspot_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.hotspots.edit', $hotspot->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('hotspot_delete')
                                                <form action="{{ route('frontend.hotspots.destroy', $hotspot->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hotspot_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.hotspots.massDestroy') }}",
