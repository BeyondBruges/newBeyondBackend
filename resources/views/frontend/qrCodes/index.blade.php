        ...
        <div class="col-md-12">
            @can('qr_code_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.qr-codes.create') }}">
                            ...
                            <tbody>
                                @foreach($qrCodes as $key => $qrCode)
                                    <tr data-entry-id="{{ $qrCode->id }}">
                                        ...
                                        <td>
                                            @can('qr_code_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.qr-codes.show', $qrCode->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('qr_code_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.qr-codes.edit', $qrCode->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('qr_code_delete')
                                                <form action="{{ route('frontend.qr-codes.destroy', $qrCode->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('qr_code_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.qr-codes.massDestroy') }}",
