        ...
        <div class="col-md-12">
            @can('feature_title_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.feature-titles.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $featureTitle->id }}">
                                        ...
                                        <td>
                                            @can('feature_title_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.feature-titles.show', $featureTitle->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('feature_title_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.feature-titles.edit', $featureTitle->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feature_title_delete')
                                                <form action="{{ route('frontend.feature-titles.destroy', $featureTitle->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feature_title_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.feature-titles.massDestroy') }}",
