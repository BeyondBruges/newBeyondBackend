        ...
        <div class="col-md-12">
            @can('user_level_question_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-level-questions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $userLevelQuestion->id }}">
                                        ...
                                        <td>
                                            @can('user_level_question_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-level-questions.show', $userLevelQuestion->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_level_question_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-level-questions.edit', $userLevelQuestion->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_level_question_delete')
                                                <form action="{{ route('frontend.user-level-questions.destroy', $userLevelQuestion->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_level_question_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-level-questions.massDestroy') }}",
