        ...
        <div class="col-md-12">
            @can('app_popup_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.app-popups.create') }}">
                            ...
                            <tbody>
                                @foreach($appPopups as $key => $appPopup)
                                    <tr data-entry-id="{{ $appPopup->id }}">
                                        ...
                                        <td>
                                            @can('app_popup_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.app-popups.show', $appPopup->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('app_popup_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.app-popups.edit', $appPopup->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('app_popup_delete')
                                                <form action="{{ route('frontend.app-popups.destroy', $appPopup->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('app_popup_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.app-popups.massDestroy') }}",
