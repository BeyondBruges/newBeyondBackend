        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.userLevelObject.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.user-level-objects.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="level_object_id" id="level_object_id" required>
                                @foreach($level_objects as $id => $entry)
                                    <option value="{{ $id }}" {{ old('level_object_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
